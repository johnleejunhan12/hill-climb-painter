...
num_shapes_to_draw = 12 # range between 100 and 5000 in slider widget

# 3) Num of hill climb iterations (dual slider)
min_hill_climb_iterations = 1000000
max_hill_climb_iterations = 1000000
...
initial_random_rectangle_pixel_width = 75
...
fail_threshold_before_terminating_hill_climb = 1000
...
def paint_target_image(target_image_full_filepath, png_output_folder_full_path, filename_of_exported_png):
    ...
    for shape_index in range(num_shapes_to_draw):
        ...
        num_hill_climb_iterations = min_hill_climb_iterations
        ...
        for _ in range(num_hill_climb_iterations):
            ...
            if fail_count > fail_threshold_before_terminating_hill_climb and is_prematurely_terminate_hill_climbing_if_stuck_in_local_minima:
            ...
            if new_score > highscore:
                ...
                if is_display_rectangle_improvement:
                    intermediate_canvas = 1


                    # Find the before error:
                    # Before error = root square error of the current canvas and target image
                    before_error = np.sqrt(np.sum((current_rgba[:,:,0:3] - target_rgba[:,:,0:3]) ** 2, axis=2)) 

                    # plt.style.use('dark_background')
                    # plt.figure(figsize=(10, 8))
                    # plt.imshow(before_error, cmap='viridis')
                    # plt.colorbar()
                    # plt.show()


                    # Draw the texture on the canvas
                    after_canvas = draw_texture_on_canvas(texture_greyscale_alpha, current_rgba.copy(), scanline_x_intersects_mutated, y_min_mutated, rgb_of_best_rect, *best_rect_list)

                    # plt.style.use('dark_background')
                    # plt.figure(figsize=(10, 8))
                    # plt.imshow(after_canvas)
                    # plt.colorbar()
                    # plt.show()
                    ...
                    # After error = root square error of the canvas with the texture drawn on it and target image
                    after_error = np.sqrt(np.sum((after_canvas[:,:,0:3] - target_rgba[:,:,0:3]) ** 2, axis=2))


                    # plt.style.use('dark_background')
                    # plt.figure(figsize=(10, 8))
                    # plt.imshow(after_error, cmap='viridis')
                    # plt.colorbar()
                    # plt.show()



                    # Get the score which is = before_error - after_error
                    score_array = before_error - after_error
                    # what is the shape of the score array?
                    total_score = round(np.sum(score_array), 2)
                    print("total_score", total_score, "iteration", _)
                    # Visualize the difference
                    # plt.style.use('dark_background')
                    # plt.figure(figsize=(10, 8))
                    # plt.imshow(score_array, cmap='RdYlGn')
                    # plt.colorbar()
                    # plt.title(f'Score = {total_score}', fontsize=14, pad=20)
                    # plt.show()




                    def array_to_rdylygn_rgb(array: np.ndarray) -> np.ndarray:
                        """
                        Convert a 2D numpy array with positive and negative values to an RGB image using RdYlGn colormap.

                        Assumes `from matplotlib import *` has been called.

                        Parameters:
                            array (np.ndarray): 2D input array with both positive and negative values.

                        Returns:
                            np.ndarray: RGB image (H, W, 3) with values in [0, 1] corresponding to RdYlGn colormap.
                        """
                        if array.ndim != 2:
                        ...
                        # Map normalized array to RGBA, then slice to RGB
                        rgba = cmap(norm(array))
                        return rgba
                    # Convert the score array to RGB using RdYlGn colormap
                    rgba_score_rdylgn = array_to_rdylygn_rgb(score_array)
                    # Enqueue the frame to gif creator
                    combined = np.concatenate((rgba_score_rdylgn, after_canvas), axis=1)

                    # Create matplotlib plot with the combined image and title (dark mode)
                    plt.style.use('dark_background')
                    fig, ax = plt.subplots(figsize=(12, 5), facecolor='black')
                    ax.imshow(combined)
                    ax.set_title(f"Total Score: {int(total_score)}", fontsize=16, pad=20, color='white')
                    ax.axis('off')  # Hide axes for cleaner look
                    ax.set_facecolor('black')
                    
                    # Convert matplotlib figure to RGBA array
                    fig.canvas.draw()
                    # Get the RGBA buffer from the figure
                    buf = np.frombuffer(fig.canvas.tostring_argb(), dtype=np.uint8)
                    buf = buf.reshape(fig.canvas.get_width_height()[::-1] + (4,))
                    # Convert from ARGB to RGBA
                    rgba_plot = buf[:, :, [1, 2, 3, 0]] / 255.0
                    plt.close(fig)  # Close the figure to free memory
                    
                    gif_creator.enqueue_frame(rgba_plot)
